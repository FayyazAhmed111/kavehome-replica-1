...
export default function FullGallery({ open, setOpen, activeIndex, setActiveIndex }) {
    const images = [
        "https://d.media.kavehome.com/image/upload/w_1900,f_auto/v1753360650/products/S81330ZF06_1V01.jpg",
        "https://d.media.kavehome.com/image/upload/w_1900,f_auto/v1754907484/ambiences/A25S107_018.jpg",
        "https://d.media.kavehome.com/image/upload/w_1900,f_auto/v1753360639/products/S81330ZF06_1D01.jpg",
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen} className=" ">
            <DialogContent className="h-screen w-full z-[10000]   hidden sm:flex sm:max-w-full overflow-hidden border-none  p-0">
                {/* Close */}
                {/* <DialogClose asChild>
                    <button className="absolute right-4 top-4 z-50 rounded p-2 hover:bg-neutral-100">
                        <X className="h-5 w-5" />
                ...
                <div className="flex h-full w-full flex-col items-center justify-center bg-white">
                    <div className="relative flex w-full flex-1 items-center justify-center">
                        ...
                        >
                            ...
                            <CarouselPrevious className="absolute -left-105 top-1/2 -translate-y-1/2 bg-white/70 p-2 hover:bg-white" />
                            <CarouselNext className="absolute -right-105 top-1/2 -translate-y-1/2 bg-white/70 p-2 hover:bg-white" />
                        </Carousel>
                    </div>

                    {/* Thumbnails */}
                    <div className="fixed bottom-4 flex w-full justify-center gap-2 bg-transparent">
                        {images.map((thumb, idx) => (
                            <button
                                key={idx}
                                onClick={() => setActiveIndex(idx)}
