...
//       try {
//         const data = await client.request(GET_PRODUCT_BY_SLUG, { productSlug });
...
export default function ProductPage({ params }) {
  const { subcategory } = params; // ✅ since “amsen” is a subcategory, not a productSlug
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        console.log("🔍 Fetching product by subcategory slug:", subcategory);

        // ✅ Pass it as `slug` to match GraphQL query variable
        const data = await client.request(GET_PRODUCT_BY_SLUG, { slug: subcategory });
